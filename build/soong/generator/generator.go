...
func init() {
	android.RegisterModuleType("javanese_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = javaneseExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = javaneseExpandVariables(ctx, dep_file)
	...
	cmd := javaneseExpandVariables(ctx, String(g.properties.Cmd))
